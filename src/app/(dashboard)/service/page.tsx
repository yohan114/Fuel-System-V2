...
import { Wrench, AlertTriangle, Clock, CheckCircle2, ClipboardList, Plus } from "lucide-react";
    ...
    <div className="space-y-8">
      <div className="border-b border-white/5 pb-4 flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <Link href="/service/records" className="text-xs font-semibold text-gray-300 hover:text-white bg-white/5 hover:bg-white/10 rounded-lg px-3 py-2 flex items-center gap-1.5 whitespace-nowrap">
            <ClipboardList className="w-4 h-4" /> Records
          </Link>
          {session.role === "ADMIN" && (
            <Link href="/service/new" className="text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg px-3 py-2 flex items-center gap-1.5 whitespace-nowrap">
