...
type AssetStatus = "created" | "regenerated" | "skipped-existing" | "skipped-finalized" | "skipped-not-here" | "skipped-excluded" | "no-rate" | "error";
...
const STATUS_META: Record<AssetStatus, { icon: React.ReactNode; label: string; cls: string }> = {
  ...
  "skipped-excluded":   { icon: <SkipForward className="w-3.5 h-3.5" />,   label: "Billed elsewhere", cls: "text-gray-400" },
  ...
  function run() {
    ...
    startTransition(async () => {
      ...
      if ((res as any).error) {
        ...
        setAssets(r.assets ?? []);
        setSummary(`${r.periodKey}: ${r.created} created, ${r.regenerated} regenerated, ${r.skippedExisting} existing, ${r.skippedFinalized} locked, ${r.noRate} no rate${r.skippedExcluded ? `, ${r.skippedExcluded} billed elsewhere` : ""}${r.errors?.length ? `, ${r.errors.length} errors` : ""}.`);
        router.refresh();
      }
    });
  }
