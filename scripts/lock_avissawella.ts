...
// Lock Avissawella (AWIS) — issue every DRAFT June 2026 bill as a final invoice.
// Dry standby vehicles; clarification flags (if any) force-overridden with a
// client-approval note. Dry-run by default; pass --apply.

const APPLY = process.argv.includes("--apply");
const ADMIN = "023cee32-d4e2-4b39-b868-11fd1ce98181";
const NOTE = "Avissawella dry rental — reviewed and approved by client";
const rs = (c: number) => "Rs " + (c / 100).toLocaleString(undefined, { maximumFractionDigits: 2 });

async function main() {
  const cfg = await getBillingConfig();
  const drafts = await prisma.bill.findMany({ where: { projectCode: "AWIS", year: 2026, month: 6, status: "DRAFT" }, orderBy: { grandTotalCents: "desc" } });
  console.log(`AWIS June: ${drafts.length} DRAFT to issue (${APPLY ? "APPLY" : "dry-run"})\n`);
  let total = 0;
  for (const d of drafts) {
    const flags = billClarifyReasons(d);
    if (!APPLY) { console.log(`  ${d.assetCode.padEnd(11)} ${rs(d.grandTotalCents).padStart(14)}${flags.length ? "  [override]" : ""}`); total += d.grandTotalCents; continue; }
    ...
    console.log(`  ${d.assetCode.padEnd(11)} ${number}  ${rs(d.grandTotalCents)}${flags.length ? "  [override]" : ""}`);
    total += d.grandTotalCents;
  }
  console.log(`\nAWIS June total: ${rs(total)} across ${drafts.length} invoices.`);
