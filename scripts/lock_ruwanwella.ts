...
// Lock Ruwanwella (RUWA) — issue every DRAFT bill for March and June 2026 as a
// final invoice. Clarification flags (meter-vs-fuel variance on dry standby
// vehicles) are force-overridden with a client-approval note, consistent with
// the other locked sites. Dry-run by default; pass --apply.

const APPLY = process.argv.includes("--apply");
const ADMIN = "023cee32-d4e2-4b39-b868-11fd1ce98181";
const NOTE = "Ruwanwella dry rental — reviewed and approved by client";
const rs = (c: number) => "Rs " + (c / 100).toLocaleString(undefined, { maximumFractionDigits: 2 });

async function main() {
  const cfg = await getBillingConfig();
  const periods = [{ y: 2026, m: 3 }, { y: 2026, m: 6 }];
  let total = 0, count = 0;
  for (const { y, m } of periods) {
    const drafts = await prisma.bill.findMany({ where: { projectCode: "RUWA", year: y, month: m, status: "DRAFT" }, orderBy: { grandTotalCents: "desc" } });
    console.log(`\n=== RUWA ${y}-${String(m).padStart(2, "0")}: ${drafts.length} DRAFT ${APPLY ? "APPLY" : "dry-run"} ===`);
    for (const d of drafts) {
      const flags = billClarifyReasons(d);
      if (!APPLY) { console.log(`  ${d.assetCode.padEnd(9)} ${rs(d.grandTotalCents).padStart(14)}${flags.length ? "  [override]" : ""}`); total += d.grandTotalCents; count++; continue; }
      ...
      total += d.grandTotalCents; count++;
    }
  }
  console.log(`\nRUWA total issued: ${rs(total)} across ${count} invoices.`);
