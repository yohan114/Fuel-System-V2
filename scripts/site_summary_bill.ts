...
async function main() {
  ...
  console.log(`  ${"machine".padEnd(20)}${"days".padStart(8)}${"cons typ".padStart(13)}${"meter".padStart(12)}${"billable".padStart(13)}${"billed on".padStart(15)}` +
  ...
  for (const l of sum.lines) {
    console.log(`  ${(l.code + " / " + (l.regNo ?? "—")).padEnd(20)}${`${l.daysHere}/${l.daysInMonth}`.padStart(8)}` +
      ...
      `${`${n1(l.billableUnits)} ${l.unit}`.padStart(13)}${l.billedOn.padStart(15)}` +
