...
// Lock Batti ICDP Lot-03 (BATTI-03) — issue every DRAFT June 2026 bill.
const APPLY = process.argv.includes("--apply");
const ADMIN = "023cee32-d4e2-4b39-b868-11fd1ce98181";
const NOTE = "Batti ICDP Lot-03 dry rental — reviewed and approved by client";
const rs = (c: number) => "Rs " + (c / 100).toLocaleString(undefined, { maximumFractionDigits: 2 });

async function main() {
  const cfg = await getBillingConfig();
  const drafts = await prisma.bill.findMany({ where: { projectCode: "BATTI-03", year: 2026, month: 6, status: "DRAFT" }, orderBy: { grandTotalCents: "desc" } });
  console.log(`Batti Lot-03 June: ${drafts.length} DRAFT to issue (${APPLY ? "APPLY" : "dry-run"})\n`);
  let total = 0;
  ...
  console.log(`\nBatti Lot-03 June total: ${rs(total)} across ${drafts.length} invoices.`);
