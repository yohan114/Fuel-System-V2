...
// Lock Muthur (Mutur Plant) — issue every DRAFT June 2026 bill as a final invoice.
// Clarification flags (meter-vs-fuel variance against fuelled machines) are
// force-overridden with a client-approval note, consistent with the other locked
// sites. Deterministic order (assetCode) → stable numbering.
//
...
const NOTE = "Mutur Plant June 2026 — reviewed and approved by client";
const rs = (c: number) => "Rs " + (c / 100).toLocaleString(undefined, { maximumFractionDigits: 2 });

async function main() {
  const cfg = await getBillingConfig();
  const drafts = await prisma.bill.findMany({
    where: { projectCode: "MUTUR", year: Y, month: M, status: "DRAFT" },
    orderBy: { assetCode: "asc" },
  });
  const issued = await prisma.bill.findMany({ where: { projectCode: "MUTUR", year: Y, month: M, status: { not: "DRAFT" } } });
  console.log(`Muthur: ${drafts.length} DRAFT to issue, ${issued.length} already issued (${APPLY ? "APPLY" : "dry-run"})\n`);

  let total = issued.reduce((s, b) => s + b.grandTotalCents, 0);
  for (const d of drafts) {
    const flags = billClarifyReasons(d);
    if (!APPLY) {
      console.log(`  ${d.assetCode.padEnd(9)} ${rs(d.grandTotalCents).padStart(14)}  ${flags.length ? "[override]" : ""}`);
  ...
  console.log(`\nMuthur June total: ${rs(total)} across ${drafts.length + issued.length} invoices.`);
