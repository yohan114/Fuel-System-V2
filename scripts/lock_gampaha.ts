...
// Lock Gampaha (GAMP) — issue every DRAFT June 2026 bill as a final invoice.
// Dry standby vehicles; clarify flags force-overridden with a client note.
const APPLY = process.argv.includes("--apply");
const ADMIN = "023cee32-d4e2-4b39-b868-11fd1ce98181";
const NOTE = "Gampaha Bridge dry rental — reviewed and approved by client";
const rs = (c: number) => "Rs " + (c / 100).toLocaleString(undefined, { maximumFractionDigits: 2 });

async function main() {
  const cfg = await getBillingConfig();
  const drafts = await prisma.bill.findMany({ where: { projectCode: "GAMP", year: 2026, month: 6, status: "DRAFT" }, orderBy: { grandTotalCents: "desc" } });
  console.log(`Gampaha June: ${drafts.length} DRAFT to issue (${APPLY ? "APPLY" : "dry-run"})\n`);
  let total = 0;
  for (const d of drafts) {
    const flags = billClarifyReasons(d);
    if (!APPLY) { console.log(`  ${d.assetCode.padEnd(8)} ${rs(d.grandTotalCents)}${flags.length ? "  [override]" : ""}`); total += d.grandTotalCents; continue; }
    ...
    console.log(`  ${d.assetCode.padEnd(8)} ${number}  ${rs(d.grandTotalCents)}${flags.length ? "  [override]" : ""}`);
    total += d.grandTotalCents;
  }
  console.log(`\nGampaha June total: ${rs(total)} across ${drafts.length} invoices.`);
